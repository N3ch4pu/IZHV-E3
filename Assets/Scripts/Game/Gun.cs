...
/*        SpawnBullet(
        ...
        if (shotgun)
        {
            float spreadCoeff = shotgunSpread / shotgunBullets;

            float angle = spreadCoeff;

            for (int i = 0; i < shotgunBullets; i++)
            {
                SpawnBullet(director.position, Quaternion.Euler(director.eulerAngles.x, director.eulerAngles.y, director.eulerAngles.z + angle - shotgunSpread));
                angle += spreadCoeff;
            }
        }
        else
